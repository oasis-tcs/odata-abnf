...
//TODO: tokens for matching rule name?
// config.expect.ruleNameTokens.forEach(
//   (ruleName) => (ast.callbacks[ruleName] = ruleNameTokenCB(ruleName))
// );
grammar.rules.forEach((rule) => {
  if (
    //TODO: move this to config.yaml or even the test case files similar to Constraints?
    rule.name.endsWith("Function") ||
    rule.name.endsWith("Import") ||
    rule.name.endsWith("Name") ||
    (rule.name.endsWith("Property") &&
      rule.name !== "primitiveProperty" &&
      rule.name !== "navigationProperty")
  )
    ast.callbacks[rule.name] = matchTokenCB(rule.name);
});
config.expect.matchTokens.forEach(
  (ruleName) => (ast.callbacks[ruleName] = matchTokenCB(ruleName))
);
...
function parse(
  ...
  log.push(
    `${colors.red(message)} ${inputString.substring(
      0,
      result.maxMatched
    )}${colors.yellow(inputString.substring(result.maxMatched))}`
  );
  ...
  printNode(log, trace.toTree(false).tree, inputString);
...
function printNode(log, node, inputString) {
  log.push(
