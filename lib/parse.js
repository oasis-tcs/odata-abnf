//TODO: rework for this project
//TODO: add constraint callbacks
//TODO: comment-out AST stuff for now - would be interesting to add later
...
const testSuite = YAML.parse(
  fs.readFileSync("./abnf/odata-temporal-testcases.yaml", "utf8")
);
...
for (const tc of testSuite.TestCases) {
  ...
  if (parse(tc.Name, tc.Rule, tc.Input, tc.FailAt /*, tc.tokens*/)) successes++;
}
...
if (successes === testSuite.TestCases.length) {
  ...
  console.log(
    colors.green(
      `\nSuccess: all ${testSuite.TestCases.length} test cases passed, touched ${touchedText} rules`
    )
  );
